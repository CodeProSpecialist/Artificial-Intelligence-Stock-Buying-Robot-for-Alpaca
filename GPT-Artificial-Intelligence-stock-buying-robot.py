...
def filter_symbols_by_price_change(search_result, percentage_threshold=1.0):
    ...
    symbols_with_increase = [symbol for symbol in relevant_symbols if get_price_change_percentage(symbol) >= percentage_threshold]
...
def main():
    while True:
        try:
            ...
            gpt_search_query = "strong buy bull ETF funds for USA and New York Stock Exchange stocks"
            ...
            for symbol in symbols_with_increase:
                ...
                print(f"Symbol: {symbol}, Current Price: {current_price:.2f}, Percentage Change: {price_change_percentage:.2f}%")
